...
class episodeRepository implements episodeInterface{
    public function getEpisode($id)
    {
        ...
        {
           $data= Episode::where('volume_id',$id)->orderByRaw('episode_name', 'asc')->orderBy("episode_name",'asc')->get();
        }else{
            $data = Episode::when(request('search'),function($query){
                $query->where('episode_name','like','%'.request('search').'%');
            })->orderByRaw('episode_name', 'asc')->orderBy("episode_name",'asc')->where('volume_id',$id)->get();
